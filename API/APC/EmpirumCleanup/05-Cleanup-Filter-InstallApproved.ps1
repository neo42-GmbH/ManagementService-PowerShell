# Empirum Cleanup API - Workflow 5: Cleanup with filter NoAssignmentInstallApproved
# Starts cleanup for filtered packages without assignment but with install approval
...
Write-Host "Empirum Cleanup API - Filter Cleanup (InstallApproved)" -ForegroundColor Cyan
...
try {
    $allPackages = Get-MmsApcEmpCleanupPackageList -Action NoAssignmentInstallApproved -TenantId $TenantId

    if (-not $allPackages -or $allPackages.Count -eq 0) {
        Write-Host "No packages found for NoAssignmentInstallApproved." -ForegroundColor Green
        Write-Host "Nothing to clean up in this category." -ForegroundColor Green
        return
    }
    ...
    # Confirm
    Write-Host "`nIMPORTANT: These packages have install approvals!" -ForegroundColor Yellow
    Write-Host "Make sure they really should be cleaned up." -ForegroundColor Yellow
    ...
    $jobId = New-MmsApcEmpCleanupJob -TenantId $TenantId -CleanupAction NoAssignmentInstallApproved
